...
// Implement PartialEq for RoomContent
impl PartialEq for RoomContent {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            (RoomContent::Monster(a), RoomContent::Monster(b)) => {
                a.ptr_eq(b) // Compare if both point to the same Rc
            }
            (RoomContent::Treasure(a), RoomContent::Treasure(b)) => a == b,
            (RoomContent::Empty, RoomContent::Empty) => true,
            _ => false,
        }
    }
}
...
fn main() {
    ...
    while player.is_alive() {
        ...
        if dungeon.current_room() == &dungeon.rooms[dungeon.rooms.len() - 1] {
            println!("You have cleared the dungeon. Congratulations!");
            break;
        }
    }
